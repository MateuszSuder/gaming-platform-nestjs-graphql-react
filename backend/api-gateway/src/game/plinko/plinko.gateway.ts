...
import { PlinkoInitCommand } from './commands/impl/plinko-init.command';
import { PlinkoStartCommand } from './commands/impl/plinko-start.command';
import { StartDto } from './dto/start.dto';
import { CompleteDto } from './dto/complete.dto';
import { PlinkoCompleteCommand } from './commands/impl/plinko-complete.command';
...
export class PlinkoGateway implements OnGatewayConnection {
  private readonly logger = new Logger('Plinko gateway');
  ...
  async init(@ConnectedSocket() client: Socket): Promise<WsResponse<any>> {
    ...
    try {
      const initValue = await this.commandBus.execute(
        new PlinkoInitCommand(client.data.user.id),
      );
  ...
  async start(
    ...
    try {
      const startValue = await this.commandBus.execute(
        new PlinkoStartCommand(client.data.user.id, bet),
      );

      return new GameWsResponse(Events.Start, startValue, HttpStatus.OK);
    } catch (e) {
  ...
  async complete(
    ...
    try {
      const completeValue = await this.commandBus.execute(
        new PlinkoCompleteCommand(gameId, userId),
      );
